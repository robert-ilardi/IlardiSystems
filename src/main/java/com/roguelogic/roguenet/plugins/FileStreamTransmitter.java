...
import static com.roguelogic.roguenet.RNAConstants.FILE_STREAM_TRANSMITTER_STATUS_DENIED;
import static com.roguelogic.roguenet.RNAConstants.FILE_STREAM_TRANSMITTER_STATUS_ERROR;
import static com.roguelogic.roguenet.RNAConstants.FILE_STREAM_TRANSMITTER_STATUS_GRANTED;
...
public class FileStreamTransmitter implements RNAPlugIn {
          ...
          if (items != null) {
            for (Item item : items) {
              if (SHARED_DIRECTORY_ITEM_TYPE.equals(item.getType())) {
                ...
                if (!RLStringUtils.IsNVL(itemValue) && (itemValue.equals(reqFile) || itemValue.equals(RLStringUtils.ParseDir(reqFile))) && item.isRead()) {
                  hasAccess = true;
                  break;
                }
              }
  ...
  private FileTransferRequest sendFileInfo(P2PHubMessage mesg) throws RogueNetException {
    ...
    if (plugInManager != null && plugInManager.getNetAgent() != null) {
      ...
      if (!RLStringUtils.IsNVL(reqFile)) {
        ...
        serverTransferId = RLStringUtils.GenerateTimeUniqueId() + RLStringUtils.GetRandomChars(10);
  ...
  public void abortFileTransfer(P2PHubMessage mesg) throws RogueNetException {
    ...
    if (!RLStringUtils.IsNVL(serverTransferId)) {
