...
public class FileTransferProgressDialog extends JFrame implements WindowListener {
  ...
  private Runnable dloadMonitor = new Runnable() {
    public void run() {
      ...
      if (dload != null) {
        while (RLStringUtils.IsNVL(dload.getServerTransferId())) {
          RLSystemUtils.SleepTight(50); // Since we cannot be notified by dload we have to poll and sleep instead of
                                        // properly waiting :(
        }
        ...
        while (!dload.isAborted() && dload.getCurrentSize() < dload.getTotalSize()) {
          ...
          if (now > (prevNow + 1000)) {
            ...
            speedLbl.setText("Avg. Transfer Rate: " + RLStringUtils.FormatDouble(tRate, 2) + " KBps");

            elapsedTimeLbl.setText("Elapsed Time: " + RLStringUtils.HumanReadableTime((long) (now - startTime)));
          }
        }
        ...
        RLSystemUtils.Sleep(5); // Wait a few seconds
  ...
  private void initDownloadMonitor() {
    ...
    initComponents((dload.isReverse() ? "Uploading..." : "Downloading..."), (dload.isReverse() ? "Uploading: " : "Downloading: ") + (dload != null ? dload.getRemoteFile() : "[UNKNOWN]"),
        "File Size: " + (dload != null ? String.valueOf(dload.getTotalSize()) + " Bytes" : "[UNKNOWN]"));
  }
  ...
  private void abortReverseDownload() throws RogueNetException {
    ...
    if (!RLStringUtils.IsNVL(dload.getRemoteFile())) {
