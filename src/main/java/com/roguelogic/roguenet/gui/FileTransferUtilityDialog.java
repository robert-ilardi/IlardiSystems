...
public class FileTransferUtilityDialog extends JFrame implements WindowListener, SynchronousTransactionRequestor {
  ...
  public void loadRemoteDirectories(P2PHubPeer agent) throws RogueNetException {
    ...
    if (agent != null && plugInManager != null) {
      ...
      if (stReceiver != null) {
        ...
        if (!RLStringUtils.IsNVL(usdListStr)) {
  ...
  public void loadRemoteFiles(P2PHubPeer agent, String remoteDir) throws RogueNetException {
    ...
    if (agent != null && plugInManager != null) {
      ...
      if (stReceiver != null) {
        ...
        if (!RLStringUtils.IsNVL(remoteFileListStr)) {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        ...
        try {
          ...
          if (!verifySSO(agent)) {
            JOptionPane.showMessageDialog(FileTransferUtilityDialog.this,
                "NOT Logged into Agent '" + agent.getUsername() + "'! Please use the Single Sign On option to logon to Agent before continuing.", FileTransferUtilityDialog.this.getTitle(),
          ...
          if (dirLst.getSelectedIndex() >= 0 && fileLst.getSelectedIndex() >= 0) {
            ...
            if (!RLStringUtils.IsNVL(localFilePath)) {
  ...
  private boolean verifySSO(P2PHubPeer agent) throws RogueNetException {
    ...
    if (agent != null && plugInManager != null && plugInManager.getEntitlementsManager() != null && plugInManager.getEntitlementsManager().getSSOController() != null) {
      ssoOk = !RLStringUtils.IsNVL(plugInManager.getEntitlementsManager().getSSOLedger().getSessionToken(agent.getUsername()));
    }
        ...
        try {
          ...
          if (dirLst.getSelectedIndex() >= 0) {
            ...
            if (!RLStringUtils.IsNVL(localFilePath)) {
              ...
              remoteFile = RLStringUtils.ParseFileFromPath(localFilePath);
  ...
  public void performFileRename() {
    ...
    try {
      ...
      if (dirLst.getSelectedIndex() >= 0 && fileLst.getSelectedIndex() >= 0) {
        ...
        if (!RLStringUtils.IsNVL(newFilename)) {
    ...
    if (agent != null && plugInManager != null) {
      ...
      if (stReceiver != null) {
        ...
        if (!FILE_SHARE_SERVICE_STATUS_SUCCESS.equals(status)) {
          JOptionPane.showMessageDialog(FileTransferUtilityDialog.this, "Remote File Rename FAILED!", FileTransferUtilityDialog.this.getTitle(), JOptionPane.ERROR_MESSAGE,
              new ImageIcon(FileTransferUtilityDialog.this.getIconImage()));
        }
  ...
  private void deleteFile(P2PHubPeer agent, String remoteDir, String remoteFile) throws RogueNetException {
    ...
    if (agent != null && plugInManager != null) {
      ...
      if (stReceiver != null) {
        ...
        if (!FILE_SHARE_SERVICE_STATUS_SUCCESS.equals(status)) {
          JOptionPane.showMessageDialog(FileTransferUtilityDialog.this, "Remote File Deletion FAILED!", FileTransferUtilityDialog.this.getTitle(), JOptionPane.ERROR_MESSAGE,
              new ImageIcon(FileTransferUtilityDialog.this.getIconImage()));
        }
