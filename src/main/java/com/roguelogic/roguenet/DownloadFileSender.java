...
public class DownloadFileSender {
  ...
  // public static final int MAX_SEGMENT_LEN = 16384; //10240; //16384; //8192;
  // //4096;
  ...
  private Runnable fsRunnable = new Runnable() {
    public void run() {
      ...
      try {
        ...
        while (doSending) {
          curReq = getNextRequest(); // Will block until a request is available or if doSending is false, will wake
                                     // up and return null
  ...
  public void scheduleFileTransfer(FileTransferRequest req) {
    synchronized (afrqLock) {
      // Slow but it works. Since "getting" is easier with an array list then a
      // HashSet
  ...
  /**
   * Get's the next FileTransferRequest from the active request queue using the
   * Round Robin approach to service the queue. Will block until a request is
   * available or if doSending is false, will wake up and return null.
   * 
    ...
    if (piManager != null && piManager.getNetAgent() != null) {
      ...
      if (len > 0) {
        ...
        if (dload != null) {
          ...
          if (RLStringUtils.IsNVL(dload.getServerTransferId())) {
