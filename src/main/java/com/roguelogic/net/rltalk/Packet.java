...
public class Packet {
  ...
  public Packet(byte[] packetData) throws RLTalkException {
    ...
    if (packetData.length - ProtocolConstants.PACKET_HEADER_LENGTH == expectedLen) {
      ...
      throw new RLTalkException(
          "Actual Data Length not equal to Expected Data Length! (Actual = " + (packetData.length - ProtocolConstants.PACKET_HEADER_LENGTH) + " ; Expected = " + expectedLen + ")");
    }
  }
  ...
  /**
   * @param multiplexerIndex
   *          The multiplexerIndex to set.
   */
