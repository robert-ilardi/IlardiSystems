...
public class TimeBasedScheduler implements Runnable {
  ...
  public void run() {
    try {
      ...
      while (scheduling) {
        ...
        RLSystemUtils.Sleep(SLEEP_INTERVAL);
  ...
  /**
   * Sets the schedule using a supplied int array where each group of two elements
   * of the array represent HH and mm (24 hour clock). Example: [0] = 20 [1] = 15.
   * This would mean 22:15
   * 
   * @param schedule
   */
  ...
  /**
   * Sets the schedule using a supplied String array where each element of the
   * array is a time in the format of HH:mm (24 hour clock).
   * 
   * @param times
   */
