/*
 * Copyright 2012 Robert C. Ilardi Licensed under the Apache License, Version
...
public class DoNothingStandaloneDaemon {
  ...
  /*
   * I personally like having a single property file for the configuration of all
   * my batch jobs and utilities. In my professional projects, I actually have a
   * more complex method of properties management, where all properties are stored
   * in a database table, and I have something called a Resource Bundle and
   * Resource Helper facility to manage it.
   *
   ...
   * manage all configuration data for the Standalone Process Template. Feel free
   * to replace this field with a more advanced configuration management mechanism
  ...
  /*
   * If this variable is set to true, any exception caused in the cleanup routine
   * will cause the entire process to exit non-zero.
   *
   * However in my professional experience, we usually just want to log these
   * exceptions, perhaps even email them to the team for investigation later, and
   * allow the process to exit ZERO, so that the batch job scheduler can continue
   * onto the next job, especially is the real execution is completed.
   */
  ...
  /*
   * A flag with tells the start and stop methods if the execution loop thread has
   * started or not.
   */
  ...
  /*
   * This field is used as a counter for the number of processing loop iterations.
   * For debugging, logging, and even custom logic implementation purposes, this
   * is a nice piece of information to have.
   */
  private long loopIterationCnt;

  /*
   * This is the file path for the stop file watcher to watch. When the stop file
   * watcher thread finds the stop file at this location, it will gracefully
  ...
  /*
   * This flag tells the start, stop file watcher methods if the file watcher loop
   * is running.
   */
   ...
   * Use this to add reads from the appProps object. 2. customProcessInit() - Use
   * this to customize your process before the execution logic runs.
   *
   ...
   * professional experience are good log messages for batch jobs or utilities to
   * print out, such as the execution timing information. This is especially
   * useful for long running jobs. You can eventually take average over the course
   * of many runs of the batch job, and then you will know when your batch job is
   * behaving badly, when it's taking too long to finish execution.
   */
  ...
  /**
   * Because we aren't using a more advanced mechanism for properties management,
   * I have included this method to allow the main() method to set the path to the
   * main properties file used by the batch jobs.
   *
   * In my professional versions of this template, this method is embedded in the
   * init() method which basically will initialize the Resource Helper component
   * and obtain the properties from the configuration tables instead.
   *
  ...
  /**
   * This method performs the cleanup of any JDBC connections, files, sockets, and
   * other resources that your execution process or your initialization process
   * may have opened or created.
   *
   ...
   * method, and if there's an interrupt signal or other fatal issue where somehow
   * the finally block didn't get called the Runtime shutdown hook will invoke
   * this method on System.exit...
   *
   * @throws Exception
   */
  public synchronized void cleanup() throws Exception {
    ...
    try {
      ...
      /*
       * It is in my experience that the Operating System will cleanup anything we
       * have "forgotten" to clean up. Therefore I do not want to waste my production
       * support team members time at 3AM in the morning to handle
       * "why did a database connection not close" It will close eventually, since it
       * is just a socket, and even if it doesn't we'll catch this in other jobs which
       * may fail due to the database running out of connections.
       *
       * However I usually have these exceptions emailed to our development team for
       * investigation the next day. For demo purposes I did not include my
       * Exception/Stacktrace Emailing utility, however I encourage you to add your
       * own.
       *
   ...
   *
   * Again like the other methods in this section of the class, do not modify this
   * method directly.
   *
  ...
  /**
   * This method is used to stop or actually "request to stop" the processing loop
   * thread.
   * 
  ...
  /**
   * This is the runnable implementation as an anon inner class which contains the
   * actual execution loop of the Daemon. This execution loop is what really
  ...
  /**
   * This is the runnable implementation as an anon inner class which contains the
   * Stop File Watcher loop. A Stop File Watcher is simply a standard file
   * watcher, except when it finds the target file, it will execute the daemon
   * shutdown routine. This is a form of inter-process communication via the file
   * system to enable a separate process or even a simple script to control (or at
   * least stop) the daemon process when it's running under NOHUP. You can simple
   * create a script which creates an empty file using the unix TOUCH command.
   */
   ...
   * Runtime.getRuntime().addShutdownHook(Thread t); method by adding an anonymous
   * class implementation of a thread.
   *
   * This thread's run method simply calls the Process's cleanup method.
   *
   * Whenever I create a class like this, I envision it being ran two ways, either
   * directly from the main() method or as part of a larger component, which may
   * wrap this entire class (A HAS_A OOP relationship).
   *
   * In the case of the wrapper, adding the shutdown hook might be optional since
   * the wrapper may want to handle shutdown on it's own.
   *
   */
  ...
  /**
   * This method is only provided in case you are loading properties from an input
   * stream or other non-standard source that is not a File.
   *
   ...
   * default, and my personal preference is not to make these exception fatal. But
   * I added the flexibility into the template for your usage.
   *
  ...
  // ------------------------------------------------------------------->
  /**
   * In general for performance reasons and for clarity even above performance, I
   * like pre-caching the properties as Strings or parsed Integers, etc, before
   * running any real business logic.
   *
   * This is why I provide the hook to readProperties which should read properties
   * from the appProps field (member variable).
   *
   * If you don't want to pre-cache your property values you can leave this method
   * blank. However I believe it's a good practice especially if your batch
   * process is a high speed ETL Loader process where every millisecond counts
  ...
  /**
   * After the properties are read from the readProperties() method this method is
   * called.
   *
   ...
   * configuration mistake you get a fatal exception and your process exists, and
   * your data is only partially loaded and you or your production support team
   * members have to debug not only the process but debug the portion of the data
   * already loaded make it in ok. This is extremely important if your batch
   * process interacts is real-time system components such as message publishers,
   * maybe you started publishing the updated records to downstream consumers?
   *
   ...
   * objects/resources opened as fields in your class which is a good idea in some
   * cases especially long running batch processes you need a hook to be able to
   * close these resources before the process exits.
   *
   * This is where that type of logic should be placed.
   *
   * Now you can throw any exception you like, however the cleanup wrapper method
   * will simply log these exceptions, the idea here is that, even though cleanup
   * is extremely important, the next step of the process is a System.exit and the
   * operating system will most-likely reclaim any resources such as files and
   * sockets which have been left opened, after some bit of time.
   *
   ...
   * correctly. The process still ran successfully at this point, so just exit and
   * log it.
   *
   * However if you really need to make the cleanup be truly fatal to the process
   * you will have to set treatCleanupExceptionsAsFatal to true.
   *
   * @throws Exception
   */
   ...
   * "StringUtils.java" class I extracted them and embedded them in this class for
