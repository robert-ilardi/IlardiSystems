...
public class RLStringUtils {
   ...
   * only if the String vlaue is equal to "TRUE" The test is case insensitive, and
   * the string "s" is trimmed!
   */
  ...
  /*
   * Generate some krapy Unique Id based on time and a few random Capital Letters.
   */
  ...
  public static String ReadTextFile(InputStream ins) throws IOException {
    ...
    buf = RLSystemUtils.LoadDataFromStream(ins);
  ...
  public static void PrintArray(PrintStream printer, String name, byte[] arr) {
    if (arr != null) {
      printer.println((new StringBuffer()).append("Array \"").append(name).append("\" has ").append(arr.length).append(" elements -").toString());
      for (int i = 0; i < arr.length; i++) {
        printer.println((new StringBuffer()).append(name).append("[").append(i).append("] = ").append(RLSystemUtils.GetAsciiFromByte(arr[i])).toString());
      }
    }
    else {
      printer.println((new StringBuffer()).append("Array \"").append(name).append("\" is NULL!").toString());
  ...
  public static String Stringize(StringFormats formats, Object obj) {
    StringBuffer sb = new StringBuffer();
    boolean addParens = false, isNeg = false;
    Double dOb;
    Integer iOb;
    NumberFormat nf;

    if (obj == null) {
      sb.append(formats.getNullFormat());
    }
    else if (obj instanceof String) {
      sb.append(obj);
    }
    else if (obj instanceof java.util.Date) {
      if (formats.getDateFormat() == null) {
        sb.append(obj.toString());
      }
      else {
        SimpleDateFormat sdf = new SimpleDateFormat(formats.getDateFormat());
        sb.append(sdf.format(obj));
      }
    } // End Date Formating
    else if (obj instanceof Double) {
      nf = NumberFormat.getInstance();
      nf.setGroupingUsed(formats.getCommatize());
      nf.setMaximumFractionDigits(formats.getPrecision());

      if (formats.getNegativeNumberFormat() == StringFormats.NEGATIVE_NUMBER_FORMAT_PARENTHESES && (((Double) obj).doubleValue() < 0)) {
        addParens = true;
        isNeg = true;
        dOb = new Double(Math.abs(((Double) obj).doubleValue()));
      }
      else {
        dOb = (Double) obj;
      }

      if (isNeg && formats.getNegHtmlStart() != null) {
        sb.append(formats.getNegHtmlStart());
      }
      ...
      sb.append(nf.format(dOb.doubleValue()));

      if (addParens) {
        sb.append(")");
      }

      if (isNeg && formats.getNegHtmlEnd() != null) {
        sb.append(formats.getNegHtmlEnd());
      }
      ...
      if (((Integer) obj).intValue() < 0) {
        addParens = true;
        isNeg = true;
        iOb = new Integer(Math.abs(((Integer) obj).intValue()));
      }
      else {
        iOb = (Integer) obj;
      }
      ...
      sb.append(nf.format(iOb.intValue()));
   ...
   * this method will respect a quoted field but does not require all fields to be
   * in sets of quotes!
   * 
   * @param text
   * @param splitChar
  ...
  public static String getValueForTag(String response, String tag) {
    ...
    if (RLStringUtils.IsNVL(response) || RLStringUtils.IsNVL(tag))
      return "";
