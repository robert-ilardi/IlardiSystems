...
public class JBeanGenerator {
  ...
  protected ArrayList<JBean> parseJBeans(String jbDescription) throws RLException {
    ...
    try {
      ...
      throw new RLException("An error occurred while attempting to parse the JBean Description! System Message: " + e.getMessage(), e); // I don't want to define what type of exception get's thrown so
  ...
  protected String createPackageDir(String packageName) {
    ...
    if (!RLStringUtils.IsNVL(packageName)) {
  ...
  private String getGeneratorComments(String clientStubFilename) {
    ...
    comments.append(RLStringUtils.GetTimeStamp());
    comments.append("\n *\n");
  ...
  private void updatePackages(ArrayList<JBean> jBeans) {
    ...
    if (RLStringUtils.IsNVL(overridePackage)) {
      return;
    }
    ...
    for (JBean jb : jBeans) {
      if (!RLStringUtils.IsNVL(jb.getPackageName())) {
        beanNameMap.put((new StringBuffer()).append(jb.getPackageName()).append(".").append(jb.getName()).toString(),
            (new StringBuffer()).append(overridePackage).append(".").append(jb.getName()).toString());
      }
    }
    ...
    for (JBean jb : jBeans) {
      ...
      for (JBeanField f : jb.getFields()) {
        ...
        if (!RLStringUtils.IsNVL(newName)) {
