...
public class Word32 implements Serializable {
  ...
  public synchronized void setBitAt(int index, Bit b) {
    ...
    if (curBit == b.getValue()) {
      // If the target bit is the same as the bit b being passed, we don't have to do
      // anything...
      return;
    }
  ...
  public synchronized int getBitValueAt(int index) {
    ...
    targetBit = 1;
    targetBit = shiftBuf & targetBit; // since targetBit is set to 1 and everything else is zero, if the bit at index
                                      // is 1 targetBit at this line will be 1 else it will be zero.
  ...
  public static void main(String[] args) {
    ...
    for (int i = Word32.WORD_BIT_SIZE - 1; i >= 0; i--) {
