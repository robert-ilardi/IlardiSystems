...
public class PacketQueue {
   ...
   * 
   * @param packet
   *          The packet of data to be pushed.
   */
   ...
   * 
   * @param packet
   *          The packet of data to be enqueued.
   */
  ...
  /**
   * 
   * @return Returns TRUE if reading from the front of the queue will return a
   *         Complete HTTP Header. FALSE if otherwise.
   */
  public synchronized boolean hasCompleteHttpHeader() {
    ...
    bufLen = 0;
    buf = new byte[4];
    RLSystemUtils.FillArray(buf, (byte) 0);
  ...
  /**
   * This method should be used to obtain a complete HTTP Header from the queue.
   * It MUST only be called if the hasCompleteHttpHeader() method returns TRUE.
   * 
   * @return Returns a complete HTTP Header and dequeues the exact number of bytes
   *         of the header from the queue. if a package contains header data plus
   *         additional bytes, only the header bytes will be returned and the
   *         remaining data is left as it's own package on the front of the queue.
   */
   ...
   * 
   * @param len
   *          number of bytes of data to be dequeued.
   ...
   * @return Returns TRUE if the queue contains enough bytes of data of length
   *         len. FALSE if otherwise.
   * @throws SambucaException
   */
  ...
  /**
   * 
   * @return Returns TRUE if the Queue is EMPTY (contains exactly ZERO packets).
   *         FALSE if otherwise.
   */
