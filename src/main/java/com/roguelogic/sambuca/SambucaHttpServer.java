...
/**
 * The "kernel" of the Sambuca HTTP Framework. It is the class that is used to
...
public class SambucaHttpServer implements SocketProcessorCustomizer, SocketSessionSweeper {
  ...
  /**
   * 
   * @return The port that the server instance is listening for incoming
   *         connections on.
   */
  public int getPort() {
    return port;
   ...
   * 
   * @param port
   *          The port for the server to listen for incoming connections on.
   */
  ...
  /**
   * Sets the user defined ServiceHandlerFactory Implementation. See the
   * ServiceHandlerFactory interface for more information why a Factory is used
   * instead of simply passing the class name of a ServiceHandler or just a single
   * instance of a ServiceHandler.
   * 
   * @param handlerFactory
   *          The ServiceHandlerFactory instance to be used to generate new
   *          ServiceHandler instances as needed by the server.
   */
  ...
  /**
   * A helper method provided so that the calling thread can block and wait while
   * the server is listening. Useful for "main" methods.
   * 
  ...
  /**
   * This method MUST be called to put the server into listening mode so that the
   * server can start receiving and accepting connections.
   * 
  ...
  /**
   * This method will cause the server to stop listening and shut all of it's
   * internal threading mechanisms down. It basically shuts down the server.
   * 
   * @throws InterruptedException
   */
  ...
  /**
   * This method is the implementation of the same method
   * SocketProcessorCustomizer interface which allows the server to "customize" of
   * initialize a newly created Socket Processor. Socket Processors are "worker"
   * implementations within the RogueLogic Socket Wrapper Framework which are used
   * to perform work on sockets which are read to be read from, since the
   * framework uses non-blocking IO via SocketChannels.
   */
  ...
  /**
   * A method to return the maximum Allowed Idled Connection Time. Currently this
   * method returns a static long, but it will be used in the future for when the
   * Sambuca framework allow users to customize security and other server
   * settings.
   * 
   * @return The number of milliseconds that a connection is allowed to remain
   *         idled.
   */
  ...
  /**
   * This method is used by the monitor thread to perform the monitoring logic on
  ...
  /**
   * This method is used by the HTTP SocketProcessor implementation to register
   * next socket connections so that they can be monitored.
   * 
   * @param userSession
   *          The RogueLogic socket "connection" wrapper being registered.
   */
  ...
  /**
   * This method sets the instance of the Sambuca Logger implementation to be used
   * to log events.
   * 
   * @param logger
   *          The instance of the SambucaLogger to be used by the server.
   */
  ...
  /**
   * Helper method to log string messages as LogMessages written to the log by the
   * SambucaLogger implementation. Will use System.out to print the messages if no
   * logger exists.
   * 
   * @param mesg
  ...
  /**
   * This method is the implementation of the same method in the
   * SocketSessionSweeper interface. It is used by the server to correctly close
   * down a Client Connection. socketSessionSweeper implementations are used by
   * the RogueLogic Socket Wrapper Framework so that if the Socket "connection"
   * wrapper objects which may contain name-value pairs contain values of complex
   * objects which have special or specific methods to clean them up which MUST be
   * invoked manually (not by the garbage collector), there is a user definable
   * hook which can clean up these objects properly. This is the method hook for
   ...
   * 
   * @param userSession
   *          The RogueLogic Socket "connection" wrapper that is being closed.
   */
  ...
  /**
   * 
   * @return The number of seconds this instance of the server uses to override
   *         the Socket Lingering time.
   */
  ...
  /**
   * The default time for socket lingering should really be left alone, but if you
   * really want to change it use this method... The default is -1 which means use
   * the default.
   * 
   * @param lingerTimeOverride
   *          - Number of seconds to override the default socket lingering time.
   */
   ...
   * 
   * @param reuseAddress
   *          Set to true if you want the server to reuse a socket address. The
   *          default is false.
   */
