...
/**
 * The HTTP protocol implementation of the RogueLogic Socket Wrapper Framework
...
public class SambucaHttpServerProcessor implements SocketProcessor {
  ...
  /**
   * This method is the implementation of the SocketProcess method. It is used to
   * process low level socket data and is the entry point to convert that data
   * into complete HTTP Requests. Once a complete HTTP Request is available, this
   * method is pass executing and the HTTP Request to the user defined
  ...
  public void process(SocketSession userSession, byte[] rawData) throws RLNetException {
    ...
    if (userSession.wasHandshook()) {
      ...
      // correctCorruptHeader(queue); //In case we have a HTTP client that doesn't
      // follow the rules!
   ...
   * 
   * @param handler
   *          The ServiceHandler instance to use for processing complete HTTP
   *          Requests.
   */
   ...
   * 
   * @param headerData
   *          - Raw HTTP Header Data read from the socket.
   * @param userSession
   *          - The RogueLogic Socket Wrapper Framework's "Connection"
   *          representation.
   * @return A HTTP Request object. At this point the request object may not be
   *         fully initialized. Meaning not all data may be populated in the
   *         request. Perhaps more socket reads are needed to complete the
   *         request. The process method will ensure only complete HTTP Requests
   *         are passed to the ServiceHandler implementation.
   ...
   * @param request
   *          The Request object to set the URL into.
   * @throws SambucaException
   */
   ...
   * 
   * @param headerData
   *          The raw HTTP data to be read.
   * @param startIndex
   *          The index in the headerData array to start reading
   * @param request
   *          The request object to set the Query String's name-value pair
   *          parameters into.
   * @throws SambucaException
   */
  ...
  /**
   * Splits raw name-value pair data into the separate elements and sets them into
   * the request's parameter storage.
   * 
   * @param paramData
   *          The raw parameter data to parse.
   * @param request
   *          The request object to add the parsed name-value pair into.
   * @throws SambucaException
   */
   ...
   * 
   * @param headerData
   *          The raw HTTP Header data.
   * @return The string representation of the HTTP Request command/method, such as
   *         GET or POST.
   */
  ...
  /**
   * Translates the string representation of the HTTP command/method into an
   * internal Sambuca Integer constant used/stored by the SambucaHttpRequest
   * object.
   * 
   * @param methodStr
   *          The string representation of the HTTP command/method
   * @return The translated internal Sambuca Integer constants for HTTP
   *         commands/methods. See the METHOD_* constants in the
   *         SambucaHttpRequest class for the definitions of the constants.
   *         METHOD_GET is for the HTTP Get Method and METHOD_POST is for the HTTP
   *         Post Method.
   */
  ...
  /**
   * Creates a SambucaHttpResponse object out of the RogueLogic socket
   * "connection" wrapper.
   * 
   * @param userSession
   *          The RogueLogic socket "connection" wrapper object to be used in the
  ...
  /**
   * Quick helper method to send HTTP Error Responses which do not require User
   * Defined ServiceHandler intervention.
   * 
   * @param response
   *          The HTTP Response to be used to send the Error Response to the
   *          client.
   ...
   * 
   * @param server
   *          Sets the reference to the SambucaHttpServer instance. Used for call
   *          backs to the server class itself.
   */
  ...
  /**
   * Parses HTTP Post request data to obtain the requested URL and sets it to the
   * Http Request object.
   * 
   * @param headerData
   *          The raw header data to be parsed.
   * @param request
   *          The request object to set the requested URL into.
   * @throws SambucaException
   */
   ...
   * 
   * @param headerData
   *          The header data to be parsed.
   * @param request
   *          The http request object to set the content-length header into.
   * @throws SambucaException
   */
   ...
   * 
   * @param request
   *          The HTTP Request to set the parsed header data into.
   * @param queue
   *          The Packet Queue to read HTTP data from.
   * @throws SambucaException
   */
  ...
  /**
   * A helper method to attempt to "repair" corrupted HTTP Header data. This
   * method is currently not used.
   * 
   * @param queue
   *          The queue of packets to be corrected.
   */
   ...
   * 
   * @param logger
   *          The instance of the Sambuca Logger Implementation to be used to log
   *          events.
   */
   ...
   * 
   * @param userSession
   *          The RogueLogic socket "connection" wrapper object, representing the
   *          new connection that was just opened.
   */
   ...
   * 
   * @param request
   *          The HTTP Request to be written to the log.
   */
  ...
  /**
   * A wrapper method used by the process method flow to call the ServiceHandler's
   * implementation of the handler method.
   * 
   * @param request
   *          The complete HTTP Request object to be processed.
   * @param response
   *          The HTTP Response object used write response data back to the
   *          client.
   * @throws SambucaException
   */
