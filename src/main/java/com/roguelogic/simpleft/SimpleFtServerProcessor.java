...
public class SimpleFtServerProcessor extends RLTalkSocketProcessor {
   ...
   * 
   * @see
   * com.roguelogic.net.rltalk.RLTalkSocketProcessor#_rlTalkHandle(com.roguelogic.
   * net.rltalk.CommandDataPair)
   */
  @Override
  ...
  private byte[] generateLoginMask() {
    return RLSystemUtils.GenerateRandomBytes(LOGIN_MASK_LEN);
  }
  ...
  private void processDateTime(CommandDataPair cmDatPair) throws RLNetException {
    ...
    reply.setStatusCode(STATUS_CODE_SUCCESS);
    reply.setData(RLStringUtils.QuickDateFormat("yyyy-MM-dd HH:mm:ss zzz Z"));
