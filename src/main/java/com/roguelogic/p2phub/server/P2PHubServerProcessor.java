...
public class P2PHubServerProcessor extends RLTalkSocketProcessor {
    ...
    if (recipients != null) {
      for (int i = 0; i < recipients.length; i++) {
        ...
        if (peerSession != null) {
          ...
          // We have to make sure we synchronized on the target peer's send lock, since we
          // do not
          // know who else is trying to encrypt a message to a particular peer at the same
          // time...
  ...
  private void processGetPeerList(CommandDataPair cmDatPair) throws RLNetException {
    ...
    // We have to make sure we synchronized on the target peer's send lock, since we
    // do not
    // know who else is trying to encrypt a CommandDataPair using the same peer's
    // codec at the same time...
