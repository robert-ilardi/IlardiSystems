...
public class FileTransmitter implements SocketProcessorCustomizer {
  ...
  private void readProperties() {
    ...
    if (!RLStringUtils.IsNVL(tmp)) {
      tmpArr = tmp.split(";");
      tmpArr = RLStringUtils.Trim(tmpArr);
  ...
  public void initTransportXorCodec() throws IOException {
    byte[] keyData = RLSystemUtils.LoadDataFromFile(transportXorFile);
  ...
  private String relativeizeLocalFile(String absPath) {
    ...
    if (absPath.length() >= 3 && ":/".equals(absPath.substring(1, 3))) {
      ...
      if (!RLStringUtils.IsNVL(driveLetterMapDir)) {
      ...
      // publisher.log(" >> Windows Drive Letter Detected. File Path Change: " +
      // bakUpFilePath);
    }
    else {
      relPath = absPath;
  ...
  public void transmitFile(LocalFile lf) throws RLNetException, OffSiteException, InterruptedException, IOException, IllegalBlockSizeException, BadPaddingException, InvalidKeySpecException,
    ...
    try {
      publisher.log("  >> Transferred " + bytesTrnsfrd + (bytesTrnsfrd == 1 ? " byte in" : " bytes in ") + RLStringUtils.FormatDouble(totalTime, 2) + " seconds. Avg Speed = "
          + RLStringUtils.FormatDouble(bps, 2) + " KB/sec");
    }
    catch (Exception e) {
      publisher.log(e);
    }
  ...
  public boolean localEqualsRemote(String localPath) throws RLNetException, InterruptedException, OffSiteException {
    ...
    synchronized (syncReqLock) {
      ...
      // same = (localFile.length() == remoteFileSize && localFile.lastModified() ==
      // remoteFileTouchTs);
