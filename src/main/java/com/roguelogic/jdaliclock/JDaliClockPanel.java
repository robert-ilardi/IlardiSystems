...
public class JDaliClockPanel extends JPanel implements MouseInputListener {
  ...
  private void drawDigits(Graphics g) {
    ...
    /*
     * for (int i = 0; i < MAX_CLOCK_DIGITS; i++) {
     * digits[i].setCurrentDigit(getCurrentClockDigitAt(i)); digits[i].draw(g); }
     */
  ...
  public void showAbout() {
    try {
      javax.swing.SwingUtilities.invokeLater(new Runnable() {
        public void run() {
          JOptionPane.showMessageDialog(JDaliClockPanel.this,
              (new StringBuffer()).append(Version.GetInfo()).append("Using Digit Class: ").append((RLStringUtils.IsNVL(digitOverrideClass) ? LedDigit.class.getName() : digitOverrideClass)).toString(),
              Version.APP_TITLE, JOptionPane.INFORMATION_MESSAGE);
        }
      });
    }
    catch (Exception e) {
  ...
  private Digit createDigit() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
    ...
    if (RLStringUtils.IsNVL(digitOverrideClass)) {
      d = new LedDigit();
    }
    else {
